...
/**
 * Asynchronously update existing content or create new content on the server
 * (PUT or POST)
 *
 * Uses fetch to send data as JSON to the server and returns the response as
 * JSON. Again remember that async function always returns a Promise.
 *
 * @param {string} url resource url on the server
 ...
 *
 * Uses fetch to send the request to the server and returns the response as
 * JSON. Again remember that async function always returns a Promise.
 *
 * @param {string} url resource url on the server
 * @returns {Promise<*>} promise that resolves to the parsed JSON
 ...
 * @param {boolean} isSuccess whether the message describes a success or a
 *     failure
 */
const createNotification = (message, containerId, isSuccess = true) => {
  ...
  // Create unique id for the notification so that it can easily be removed
  // after timeout
  ...
  // Add message test inside the paragraph and append the paragraph to the
  // container
...
const removeElement = (containerId, elementId) => {
  const container = document.getElementById(containerId);
  container.querySelectorAll(`#${elementId}`)
...
const decreaseProductCount = productId => {
  ...
  if (productCount > 1) {
    ...
    // use sessionStorage's setItem('key', 'value')
    // (https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage#basic_usage)
    // to remove 1 from the product amount in the cart
    //    key: productId
    //    data: productCount - 1
    // Implementation under
...
const getAllProductsFromCart = () => {
  return Object.keys(sessionStorage).reduce((array, str) => {
    const item = {name: str, amount: sessionStorage.getItem(str)};
    return [...array, item];
  }, []);
};
