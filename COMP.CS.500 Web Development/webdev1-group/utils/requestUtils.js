 ...
 * @returns {Array|null} array [username, password] from Authorization header,
...
const getCredentials = request => {
  ...
  //       You need to first decode the header back to its original form
  //       ("email:password").
...
const acceptsJson = request => {
  // Check if the client accepts JSON as a response based on "Accept" request
  // header
  //  NOTE: "Accept" header format allows several comma separated values
