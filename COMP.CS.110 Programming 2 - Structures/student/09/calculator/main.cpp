...
{
    std::string tocaps = str;
    int length = str.length();
    for ( int i = 0; i < length; i++ ) {
        tocaps.at(i) = toupper(str.at(i));
    }
    return tocaps;
}
...
int main() {
    ...
    while ( true ) {
        ...
        std::vector<Command>::const_iterator iter = COMMANDS.begin();
        bool command_found = false;
        while ( iter != COMMANDS.end() ) {
            std::string command_str = iter->str;
            if ( to_upper(command_to_be_executed) == command_str ) {
                command_found = true;



                // tarkistetaan parametrien määrä
                if ( pieces.size()-1 != iter->parameter_number ) {
                ...
                // suoritetaan komento
                double para1, para2;
                if ( not string_to_double(pieces.at(1), para1) or
                    not string_to_double(pieces.at(2), para2) ) {
                    std::cout << "Error: a non-number operand." << std::endl;
                    break;
                }
                std::cout << iter->action(para1, para2) << std::endl;
            }
            iter++;
        }
