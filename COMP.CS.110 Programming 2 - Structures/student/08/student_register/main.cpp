...
bool student_number_found(const std::map< std::string, Student* >
                              student_numbers, std::string student)
{
...
void rewrite_file(std::string file_name, const std::map< std::string, Student* >
                                             user_ids)
{
    std::string data_out = file_name;
    std::ofstream file;
    file.open(data_out);
...
int main() {
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            if (not student_number_found(student_numbers, parts[1])) {
                continue;
            }
            ...
            rewrite_file(file_name, user_ids);
